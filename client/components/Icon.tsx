...
interface IconProps {
  name: 'like' | 'comment' | 'share' | 'google' | 'close' | 'home' | 'pen' | 'book' | 'library' | 'plus' | 'collapse' | 'expand' | 'login' | 'language' | 'sun' | 'moon' | 'search' | 'document' | 'arrow-up' | 'trophy' | 'newspaper' | 'book-open' | 'scroll' | 'chevron-left' | 'chevron-right' | 'volume-up' | 'volume-off' | 'quote' | 'school' | 'star' | 'lightning' | 'landmark' | 'linkedin' | 'instagram' | 'youtube' | 'facebook' | 'twitter' | 'edit' | 'collection' | 'archive' | 'bulb' | 'settings' | 'calendar' | 'printer' | 'leaf' | 'trash';
  isFilled?: boolean;
}

export const Icon: React.FC<IconProps> = ({ name, isFilled = false }) => {
  const icons: Record<IconProps['name'], React.ReactNode> = {
    ...
    google: (
        <svg className="w-5 h-5" aria-hidden="true" focusable="false" data-prefix="fab" data-icon="google" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 488 512"><path fill="currentColor" d="M488 261.8C488 403.3 381.5 512 244 512 110.3 512 0 398.5 0 256S110.3 0 244 0c73.5 0 134.3 29.1 180.5 73.4l-71.2 69.9C326.6 113.5 289.4 92.3 244 92.3c-88.6 0-160.2 72.3-160.2 161.5s71.6 161.5 160.2 161.5c97.1 0 134.2-61.5 141-92.3h-141v-74.8h272.9c1.5 16.3 2.5 33.9 2.5 52.8z"></path></svg>
    ),
    close: (
    ...
    quote: (
       <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
         <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12.375m0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
       </svg>
    ),
    library: (
       <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
         <path strokeLinecap="round" strokeLinejoin="round" d="M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6M3 9v11.25a1.5 1.5 0 001.5 1.5h15a1.5 1.5 0 001.5-1.5V9M3 9h18" />
       </svg>
    ),
    collection: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
    ...
    language: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5h12M9 3v2m4 13l4-4M19 17v2m-4-13a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2zM3 13h4m-4 4h4m4-4h4m-4 4h4M13 3v2m0 13a9 9 0 01-9-9" /></svg>
    ),
    sun: (
    ...
    'trophy': (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-3.375c0-.621-.504-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0V5.25m-5.007 0V5.25m5.007 0v-2.25h-5.007v2.25m0 0h-1.125a3 3 0 00-3 3v2.25a3 3 0 003 3h1.125m6.132-8.25h1.125a3 3 0 013v2.25a3 3 0 01-3 3h-1.125" />
        </svg>
    ),
    'bulb': (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 18.5v-3m0-12a6 6 0 0 0-6 6c0 1.657.672 3.158 1.757 4.243.868.868 1.493 2.007 1.743 3.257a1.5 1.5 0 0 0 1.49 1.25h2.02c.756 0 1.391-.56 1.49-1.312.234-1.743.844-3.332 1.95-4.438A6.002 6.002 0 0 0 12 3.5z" />
        </svg>
    ),
    'settings': (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
